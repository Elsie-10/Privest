// lib/midnight.ts
//
// WHAT'S REAL HERE: nothing is sent over the network by this file itself —
// it only drives the staged progress animation. As of the Python backend
// migration, the actual analysis step that follows this animation (see
// PrivacyFlow.tsx's onComplete) DOES send the parsed transactions to the
// FastAPI backend over the network (POST /api/portfolio/metrics), so the
// "stays local" framing this module previously implied no longer holds
// end-to-end — see root README "Architecture & privacy".
//
